...
#pragma mark - MHUB get pOwer State
+(NSURL*) getMHUBPowerState:(NSString*)strAddress {
    return [NSURL URLWithString:[NSString stringWithFormat:APIV2_GETPOWER_STATE, strAddress]];
}

#pragma mark - MHUB set pOwer State
+(NSURL*) setMHUBPowerState:(NSString*)strAddress flag:(NSString *)powerState{
    return [NSURL URLWithString:[NSString stringWithFormat:APIV2_SETPOWER_STATE, strAddress,powerState]];
}
//+(NSString *) getMHUBPowerState:(NSString*)strAddress {
//    return [NSString stringWithFormat:APIV2_GETPOWER_STATE, strAddress];
//}
...
+(NSString*) Mhub_DashUpgrade:(NSString*)strAddress mosVersion:(float )mosVersionNo {
    ...
    {
         return [NSString stringWithFormat:@"%@%@", [API getBaseURL:strAddress], FILE_DASHUPGRADE_CGI];
    }
...
{
    return [NSString stringWithFormat:@"%@%@", [API getBaseURL:strAddress], FILE_CONNECTIVITY_JSON];
}
...
#pragma mark - WIFI SETUP API's
+(NSString*) getWifiMode:(NSString*)strAddress  {
    return [NSString stringWithFormat:@"%@%@", [NSString stringWithFormat:BASEURLIP, strAddress], API_GET_WIFIMODE];
}

+(NSString*) setAPMode:(NSString*)strAddress  {
    return [NSString stringWithFormat:@"%@%@", [NSString stringWithFormat:BASEURLIP, strAddress], API_SET_APMODE];
}


+(NSString*) networkScan:(NSString*)strAddress  {
    return [NSString stringWithFormat:@"%@%@", [NSString stringWithFormat:BASEURLIP, strAddress], API_NETWORK_SCAN];
}

+(NSString*) setSTAModeURL:(NSString*)strAddress  {
    return [NSString stringWithFormat:@"%@%@", [NSString stringWithFormat:BASEURLIP, strAddress], API_SET_STA_MODE];
}
...
+(NSURL*) getGlobalAVRIRDetails205:(NSString*)strAddress {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", [API getBaseURL:strAddress], APIV2_GETGLOBALIRPACK205]];
}
+(NSURL*) getZPCECPACKURL:(NSString*)strAddress {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", [API getBaseURL:strAddress], APIV2_GETCECPACK]];
}

+(NSURL*) getCECJSONPACKURL:(NSString*)strAddress {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", [API getBaseURL:strAddress], APIV2_GETCECJSON]];
}
...
+(NSURL*) switchInURL:(NSString*)strAddress OutputIndex:(NSInteger)intOutputIndex InputIndex:(NSInteger)intInputIndex {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/%ld", [API getBaseURL:strAddress], APIV2_SWITCHING, [Utility integerToCharacter:intOutputIndex], (long)intInputIndex]]; // [ox]/[iy]/       where [ox] = output (a,b,c…..), [iy] = input (1,2,3…..)
}
...
+(NSURL*) switchInAudio411ARCURL:(NSString*)strAddress outputFlag:(NSString *)outputIndex InputFlag:(NSString*)inputIndex{
    NSString *strURl = [NSString stringWithFormat:APIV2_ARC_SWITCHING_AUDIO,outputIndex];
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@", [API getBaseURL:strAddress],strURl ,inputIndex]];
}

+(NSURL*) controlVolumeURL:(NSString*)strAddress OutputIndex:(NSInteger)intOutputIndex Volume:(NSInteger)intVolume {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/%ld", [API getBaseURL:strAddress], APIV2_CONTROLVOLUME, [Utility integerToCharacter:intOutputIndex], (long)intVolume]]; // [ox]/[vy]/       where [ox] = output (a,b,c…..), [iy] = volume (1-100)
}
...
+(NSURL*) controlMuteURL:(NSString*)strAddress OutputIndex:(NSInteger)intOutputIndex Mute:(BOOL)isMuted {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/%@", [API getBaseURL:strAddress], APIV2_CONTROLMUTE, [Utility integerToCharacter:intOutputIndex], isMuted ? @"true" : @"false"]]; // [ox]/[mx]/       where [ox] = output (a,b,c…..), [mx] = mute state (true=muted(no audio), false=unmuted(audio))
}
...
+(NSURL*) executeSuperSequenceURL:(NSString*)strAddress SequenceId:(NSString*)strSequenceId superSeqFlag:(BOOL)isFlag {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/%@", [API getBaseURL:strAddress], APIV2_EXECUTESEQUENCE, strSequenceId,isFlag ? @"true" : @"false"]]; // [sid]/           where [sid] = Sequence ID
}

//MARK: Execute Function APIs
+(NSURL*) executeFunctionURL:(NSString*)strAddress functionId:(NSString*)strFunctionId Flag:(BOOL)isFlag{
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/%@", [API getBaseURL:strAddress], APIV2_EXECUTEFUNCTION, strFunctionId,isFlag ? @"true" : @"false"]];
}
...
+(NSURL*) executeCommandURL:(NSString*)strAddress PortId:(NSInteger)intPortId CommandId:(NSInteger)intCommandId TouchType:(TouchActivity)objTouchType{
    ...
    switch (objTouchType) {
        case Normal:
            urlObj = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%ld/%ld", [API getBaseURL:strAddress], APIV2_EXECUTECOMMAND, (long)intPortId, (long)intCommandId]];
            break;
        case Hold:
            urlObj = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%ld/%ld/%@", [API getBaseURL:strAddress], APIV2_EXECUTECOMMAND, (long)intPortId, (long)intCommandId,kHold]];
            break;
        case Release:
            urlObj = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%ld/%ld/%@", [API getBaseURL:strAddress], APIV2_EXECUTECOMMAND, (long)intPortId, (long)intCommandId,kRelease]];
            break;
        default:
            urlObj = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%ld/%ld", [API getBaseURL:strAddress], APIV2_EXECUTECOMMAND, (long)intPortId, (long)intCommandId]];
            break;
...
+(NSURL*) executeCommandURL_AcceptString:(NSString*)strAddress PortId:(NSString *)intPortId CommandId:(NSInteger)intCommandId {
    ...
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/%ld", [API getBaseURL:strAddress], APIV2_EXECUTECOMMAND_CEC, intPortId, (long)intCommandId]]; // [io]/[cy]/       where [io] = IR port ID (1,2,3…..), [cy] = IR command number
}
