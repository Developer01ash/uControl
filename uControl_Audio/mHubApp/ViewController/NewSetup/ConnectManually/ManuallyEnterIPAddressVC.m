            ...
            if (objSelectedMHubDevice.Generation == mHub4KV3) {
                ...
                [APIManager fileAllDetails:objSelectedMHubDevice completion:^(APIV2Response *responseObject) {
                    if (responseObject.error) {
                        ...
                        if(objHubResp.isPaired)
                        {
                            NSMutableArray *tempArrOfSlave = [[NSMutableArray alloc]init];
                            tempArrOfSlave = [Hub getObjectArrayFromPairJSON:objHubResp.PairingDetails];
                            for(int i = 0 ; i < tempArrOfSlave.count ;i++)
                            {
                                Hub *objHub = (Hub *)[tempArrOfSlave objectAtIndex:i];
                                    objHub.BootFlag  = true;
                                    objHub.InputCount  = 8;
                                    objHub.OutputCount  = 8;

                            }
                            [self validationToNavigateToConfirmationScreen:objHubResp Stacked:true Slave:tempArrOfSlave];
                        }else{
                            if(self.setupType  == HDA_ManualIP_WithoutType){
