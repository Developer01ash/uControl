...
- (void)viewDidLoad {
    ...
    [self.systemTypeToBuild setTextColor:colorWhite];
    [self.typeMsg setTextColor:colorMiddleGray_868787];
    ...
    switch (self.setupType) {
        case HDA_SetupVideo: {
            ...
            self.arrConnectionData = [[NSMutableArray alloc] initWithObjects:@"STANDALONE", nil];
            break;
        }
        ...
        case HDA_SetupAudio: {
            ...
            if ([objSearch.arrItems count] > 1 || [AppDelegate appDelegate].systemType == HDA_ConnectManually) {
                [self.arrConnectionData addObject:@"CREATE STACKED SYSTEM"];
            }
            break;
        }
        ...
        case HDA_SetupVideoAudio: {
            ...
            self.arrConnectionData = [[NSMutableArray alloc] initWithObjects:@"CREATE STACKED SYSTEM", nil];
            break;
        }
...
-(void) viewWillAppear:(BOOL)animated {
    @try {
        ...
        if ([AppDelegate appDelegate].deviceType == mobileSmall) {
            ...
            [self.systemTypeToBuild setFont:textFontBold12];
            [self.typeMsg setFont:textFontRegular12];
        } else {
            [self.lblHeaderMessage setFont:textFontBold18];
            [self.systemTypeToBuild setFont:textFontBold16];
            [self.typeMsg setFont:textFontRegular16];
        }
