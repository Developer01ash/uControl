...
-(void) viewWillAppear:(BOOL)animated {
    @try {
        ...
        if ([AppDelegate appDelegate].deviceType == mobileSmall) {
            ...
            [self.lblTitle setFont:textFontBold12];
            ...
            [self.lblTitle setFont:textFontBold16];
...
- (IBAction)btnSetupTypeVideoAudio_Clicked:(CustomButton *)sender {
    @try {
        ...
        bool flag_ZP_deviceFound = false;
        for (int i = 0; i < self.arrSearchData.count; i++) {
            ...
            if (objData.modelType == HDA_MHUBZP5 || objData.modelType == HDA_MHUBZPMINI ) {
                for (Hub *objHub in objData.arrItems) {
                    ...
                    flag_ZP_deviceFound = true;
                    break;
                }
            }
        }
        if (flag_ZP_deviceFound) {
            BuildAStackVC *objVC = [mainStoryboard instantiateViewControllerWithIdentifier:@"BuildAStackVC"];
