...
+(SearchData*) initWithSectionType:(HDAModelType)secType RowArray:(NSMutableArray*)arrRows {
    ...
    @try {
        ...
        switch (secType) {
            ...
            case HDA_MHUBU41140: {
                    objReturn.strTitle = kDEVICEMODEL_MHUBU41140;
                    objReturn.imgSection = objTheme.themeType == Light ? kDEVICEMODEL_IMAGE_MHUBU41140_SNOW : kDEVICEMODEL_IMAGE_MHUBU41140_CARBONITE;
                    objReturn.sectionHeight = IS_IPAD ? kDEVICEMODEL_IMAGE_HEIGHT_NEW_IPAD :  kDEVICEMODEL_IMAGE_HEIGHT_NEW;

                    break;
                }
            case HDA_MHUBZP5: {
                objReturn.strTitle = kDEVICEMODEL_MHUBZP5;
                objReturn.imgSection = objTheme.themeType == Light ? kDEVICEMODEL_IMAGE_MHUBUZP5_SNOW : kDEVICEMODEL_IMAGE_MHUBUZP5_CARBONITE;
                objReturn.sectionHeight = IS_IPAD ? kDEVICEMODEL_IMAGE_HEIGHT_NEW_IPAD :  kDEVICEMODEL_IMAGE_HEIGHT_NEW;

                break;
            }
            case HDA_MHUBZPMINI: {
                objReturn.strTitle = kDEVICEMODEL_MHUBZPMINI;
                objReturn.imgSection = objTheme.themeType == Light ? kDEVICEMODEL_IMAGE_MHUBUZP1_SNOW : kDEVICEMODEL_IMAGE_MHUBUZP1_CARBONITE;
                objReturn.sectionHeight = IS_IPAD ? kDEVICEMODEL_IMAGE_HEIGHT_NEW_IPAD :  kDEVICEMODEL_IMAGE_HEIGHT_NEW;

                break;
            }
            case HDA_MHUBS: {
                objReturn.strTitle = kDEVICEMODEL_MHUB_S;
                objReturn.imgSection = objTheme.themeType == Light ? kDEVICEMODEL_IMAGE_MHUBUZP1_SNOW : kDEVICEMODEL_IMAGE_MHUBUZP1_CARBONITE;
                objReturn.sectionHeight = IS_IPAD ? kDEVICEMODEL_IMAGE_HEIGHT_NEW_IPAD :  kDEVICEMODEL_IMAGE_HEIGHT_NEW;
...
-(void) startSearchNetwork {
    ...
    objMhub41140 = [SearchData initWithSectionType:HDA_MHUBU41140 RowArray:[[NSMutableArray alloc] init]];
    objMhubZP5 = [SearchData initWithSectionType:HDA_MHUBZP5 RowArray:[[NSMutableArray alloc] init]];
    objMhubZPMINI = [SearchData initWithSectionType:HDA_MHUBZPMINI RowArray:[[NSMutableArray alloc] init]];
    objMhubS = [SearchData initWithSectionType:HDA_MHUBS RowArray:[[NSMutableArray alloc] init]];
    objMhubU43140 = [SearchData initWithSectionType:HDA_MHUB431U RowArray:[[NSMutableArray alloc] init]];
    objMhubU86240 = [SearchData initWithSectionType:HDA_MHUB862U RowArray:[[NSMutableArray alloc] init]];
    ...
    [arrSearchData addObject:objMhub41140];
    [arrSearchData addObject:objMhubZP5];
    [arrSearchData addObject:objMhubZPMINI];
    [arrSearchData addObject:objMhubS];
    ...
    //August2020: SSDP is search mainly found V3 units and v3 will be seen in find devices and manual connect, so below condition is checking if user coming or using below option then only SSDP search will work.
    // SSDP searching
    if([[NSUserDefaults standardUserDefaults]boolForKey:@"IsComingFromFindDevices"])
    {
...
-(void) reloadSearchedData {
    @try {
        ...
        for (SearchData *objData in arrSearchData) {
                    ...
                    if (![objHub.Address isIPAddressEmpty] && ![objHub.SerialNo isNotEmpty]) {
                        ...
                        [APIManager fileAllDetails:objHub completion:^(APIV2Response *responseObject) {
                            if (!responseObject.error) {
                                ...
                                NSString *str2 = objHubAPI.Address;
                                //NSLog(@"str2 %@",str2);
            ...
            //August2020: IF we enable below code , then v3 units are allowed to work if they are single in network.
//            if (self.delegate) {
//                [self.delegate searchData:self didFindDataArray:self->arrSearchData];
//            }
        }
...
-(void) mdnsManager:(MDNSManager *)manager didFindMHUB:(Hub *)objHub {
    switch (objHub.Generation) {
        ...
        {
            if (objHub.InputCount == 4) {
                       if ([objHub.modelName isContainString:kDEVICEMODEL_MHUBU41140]) {
                           [objMhub41140.arrItems addObject:objHub];
                       }
            }
        break;
        }
        case mHubZP:
        {
            if (objHub.InputCount == 5) {
                       if ([objHub.modelName isContainString:kDEVICEMODEL_MHUBZP5]) {
                           [objMhubZP5.arrItems addObject:objHub];
                       }
            }
            else
                {
                if ([objHub.modelName isContainString:kDEVICEMODEL_MHUBZPMINI]) {
                    [objMhubZPMINI.arrItems addObject:objHub];
                }
                }
            break;
        }
        case mHubS:
        {
            [objMhubS.arrItems addObject:objHub];
            break;
        }
        default:
            break;
...
-(void) mdnsManager:(MDNSManager *)manager didRemoveMHUB:(Hub *)objHub {
    switch (objHub.Generation) {
        ...
        case mHub411:
        {
            if (objHub.InputCount == 4) {
                       if ([objHub.modelName isContainString:kDEVICEMODEL_MHUBU41140]) {
                           [objMhub41140.arrItems removeObject:objHub];
                       }
            }
        }
        case mHubZP:
        {
            if (objHub.InputCount == 5) {
                       if ([objHub.modelName isContainString:kDEVICEMODEL_MHUBZP5]) {
                           [objMhubZP5.arrItems removeObject:objHub];
                       }
            }
            else
                {
                if ([objHub.modelName isContainString:kDEVICEMODEL_MHUBZPMINI]) {
                    [objMhubZPMINI.arrItems removeObject:objHub];
                }
                }
        }
        case mHubS:
        {
            [objMhubS.arrItems removeObject:objHub];
            break;
        }
        default:
            break;
...
-(void) mdnsManager:(MDNSManager *)manager Service:(NSNetService *)service didFindAddress:(NSString *)address {
    ...
    for (SearchData *objData in arrSearchData) {
        for (Hub *objHub in objData.arrItems) {
            if ([objHub.modelName isEqualToString:service.name] && [service addresses].count > 0 && [objHub.Address isIPAddressEmpty]) {
                NSLog(@"mHubAudio objHub 22 %@ and  model name %@",address,objHub.modelName);
