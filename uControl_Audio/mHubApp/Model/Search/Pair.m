...
-(id)initWithPair:(PairDetail *)pairData {
    ...
    self.type           = pairData.type;
    self.product_code   = pairData.product_code;
    self.name   = pairData.name;
    return self;
}
...
+(PairDetail*) getObjectFromDictionary:(NSDictionary*)dictResp {
    ...
    @try {
        if ([dictResp isKindOfClass:[NSDictionary class]]) {
            ...
            objReturn.type = [Utility checkNullForKey:kTYPE Dictionary:dictResp];
            ...
            objReturn.product_code = [Utility checkNullForKey:kPRODUCT_CODE Dictionary:dictResp];
            objReturn.name    = [Utility checkNullForKey:kMACRONAME Dictionary:dictResp];
        }
...
+(PairDetail*) getObjectFromHubObject:(Hub*)objHub {
    ...
    @try {
        if ([objHub isNotEmpty] && [objHub isKindOfClass:[Hub class]]) {
            ...
            objReturn.name      = objHub.hubName_InStack;
            if ([objHub isPro2Setup]) {
                objReturn.type      = @"hybrid";
            }
            else if([objHub isZPSetup]){
                objReturn.type      = @"zp";
                }
            else{
                objReturn.type      = @"video";
                }
            objReturn.product_code = objHub.modelName;
...
-(NSDictionary*) dictionaryRepresentation {
    @try {
        ...
        [dict setValue:self.type forKey:kTYPE];
        [dict setValue:self.product_code forKey:kPRODUCT_CODE];
        [dict setValue:self.name forKey:kMACRONAME];
...
- (void)encodeWithCoder:(NSCoder *)encoder {
    @try {
        ...
        [encoder encodeObject:self.product_code forKey:kPRODUCT_CODE];
        [encoder encodeObject:self.name forKey:kMACRONAME];
...
- (id)initWithCoder:(NSCoder *)decoder {
    @try {
        if(self = [super init]) {
            ...
            self.product_code       = [decoder decodeObjectForKey:kPRODUCT_CODE];
            self.name       = [decoder decodeObjectForKey:kMACRONAME];
        }
    }
    @catch(NSException *exception) {
        [[AppDelegate appDelegate] exceptionLog:exception Function:__PRETTY_FUNCTION__ Line:__LINE__];
    ...
    @try {
        if ([objMasterHub isNotEmpty]) {
            ...
            if (![objReturn.master.unit_id isNotEmpty]) {
                ...
                if([objMasterHub isPro2Setup]){
                objReturn.master.type = @"hybrid";
                }
                else if([objMasterHub isZPSetup])
                {
                objReturn.master.type = @"zp";
                }
                else
                {
                objReturn.master.type = @"video";
                }
                objReturn.master.product_code = objMasterHub.modelName;
        }
        }
