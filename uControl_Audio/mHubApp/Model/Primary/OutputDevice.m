...
-(id)initWithOutput:(OutputDevice *)opData {
    ...
    @try {
        ...
        self.irmapId                     = opData.irmapId;
        self.isDeleted                  = opData.isDeleted;
        self.isIRPack                   = opData.isIRPack;
        self.outputType                 = opData.outputType;
        ...
        self.outputType_VideoOrAudio    = opData.outputType_VideoOrAudio;
...
+(OutputDevice*) getOutputObject_From:(OutputDevice *)objFrom To:(OutputDevice*)objTo {
    ...
    @try {
        ...
        if ([objFrom.outputType isNotEmpty]) {
            objReturn.outputType = objFrom.outputType;
        }
        if ([objFrom.outputType_VideoOrAudio isNotEmpty]) {
                  objReturn.outputType_VideoOrAudio = objFrom.outputType_VideoOrAudio;
              }
        ...
        if (objFrom.irmapId != 0) {
            objReturn.irmapId = objFrom.irmapId;
        }
...
+(OutputDevice*) getObjectFromDictionary:(NSDictionary*)dictResp Hub:(Hub*)objHub {
    ...
    @try {
        if ([dictResp isKindOfClass:[NSDictionary class]]) {
            ...
            if ([[Utility checkNullForKey:kTYPE Dictionary:dictResp] isNotEmpty]) {
                    objReturn.outputType = [Utility checkNullForKey:kTYPE Dictionary:dictResp];
            }
            ...
            //Old code. But now adding if else condition. Because from ZP devices there is new id coming that is IR map id which will be same as port id. so if irmapid is coming then port id will be same, not as we generate from index or input/output count.
           // objReturn.PortNo = objReturn.Index + objHub.InputCount;
            ...
            if ([objHub isUControlSupport]) {
                if ([objHub isAPIV2]) {
                    NSInteger portIDObj;
                    if([objHub isZPSetup]){
                        portIDObj  = objReturn.irmapId;
                    }else{
                        portIDObj = objReturn.PortNo;
                    }
                    [APIManager fileIRPackXML_Hub:objHub PortId:portIDObj completion:^(APIV2Response *responseObject) {
                        ...
                        [CommandType saveCustomObject:objReturn.objCommandType key:[NSString stringWithFormat:kDeviceIRPackDefaults, (long)portIDObj]];
                    }];
                } else {
...
+(OutputDevice*) getOutputObjectFromDictionary:(NSDictionary*)dictResp Hub:(Hub*)objHub {
    ...
    @try {
        if ([dictResp isKindOfClass:[NSDictionary class]]) {
            ...
            if ([[Utility checkNullForKey:kOUTPUTTYPE_VIDEOorAUDIO Dictionary:dictResp] isNotEmpty]) {
                              objReturn.outputType_VideoOrAudio = [Utility checkNullForKey:kOUTPUTTYPE_VIDEOorAUDIO Dictionary:dictResp];
                      }
            ...
            if ([objHub isUControlSupport]) {
                ...
                id cacheData = [CommandType retrieveCustomObjectWithKey:[NSString stringWithFormat:kDeviceIRPackDefaults, (long)portIDObj]];
...
+(OutputDevice*) getOutputObjectFromIndex:(NSInteger)intIndex OutputCount:(NSInteger)OutputCount {
    ...
    @try {
        ...
        objReturn.outputType                = @"";
        objReturn.outputType_VideoOrAudio   = @"";
        objReturn.sourceType                = Uncontrollable;
        objReturn.objCommandType            = [[CommandType alloc]init];
        objReturn.imgControlGroup           = nil;
    ...
    @try {
        if ([dictResp isKindOfClass:[NSDictionary class]]) {
           ...
           // objReturn.PortNo    = objReturn.Index+objHub.InputCount;
            ...
            id cacheCommandData = [CommandType retrieveCustomObjectWithKey:[NSString stringWithFormat:kDeviceIRPackDefaults, (long)portIDObj]];
...
+(NSMutableArray*) getObjectArraySystem:(NSMutableArray*)arrResp Hub:(Hub*)objHub keyValue:(NSString *)key_VideoAudio{
    @try {
        ...
        if([arrResp isNotEmpty]) {
            for (int i = 0; i < [arrResp count]; i++) {
                NSMutableDictionary *dictResp = [[arrResp objectAtIndex:i] mutableCopy];
                if([key_VideoAudio isEqualToString:kOUTPUT_VIDEO] || [key_VideoAudio isEqualToString:kSTACK_OUTPUT_VIDEO])
                               {
                                   [dictResp setValue:@"video" forKey:kOUTPUTTYPE_VIDEOorAUDIO];
                               }
                               else if([key_VideoAudio isEqualToString:kOUTPUT_AUDIO] || [key_VideoAudio isEqualToString:kSTACK_OUTPUT_AUDIO]){
                                   [dictResp setValue:@"audio" forKey:kOUTPUTTYPE_VIDEOorAUDIO];
                               }
                
                OutputDevice *objDevice = [OutputDevice getObjectFromSystemDictionary:dictResp Hub:(Hub*)objHub];
//                #define        @"output_video" kSTACK_OUTPUT_VIDEO
//                #define kINPUT_AUDIO        @"input_audio"
//                #define kOUTPUT_AUDIO       @"output_audio" kSTACK_OUTPUT_AUDIO
...
-(NSDictionary*) dictionaryRepresentation {
    @try {
        ...
        [dict setValue:self.outputType forKey:kTYPE];
         [dict setValue:self.outputType_VideoOrAudio forKey:kOUTPUTTYPE_VIDEOorAUDIO];
...
- (void)encodeWithCoder:(NSCoder *)encoder {
    @try {
        ...
        [encoder encodeObject:self.outputType forKey:kTYPE];
        [encoder encodeObject:self.outputType_VideoOrAudio forKey:kOUTPUTTYPE_VIDEOorAUDIO];
    }
    @catch(NSException *exception) {
        [[AppDelegate appDelegate] exceptionLog:exception Function:__PRETTY_FUNCTION__ Line:__LINE__];
    }
    ...
    @try {
        if(self = [super init]) {
            ...
            @try {
                ...
                self.irmapId     = [[decoder decodeObjectForKey:kIRMAP] integerValue];
            ...
            self.outputType                      = [decoder decodeObjectForKey:kTYPE];
            self.outputType_VideoOrAudio         = [decoder decodeObjectForKey:kOUTPUTTYPE_VIDEOorAUDIO];
        }
